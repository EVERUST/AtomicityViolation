...
impl Peers {
	...
	pub fn add_peer(&self) {
		...
		let peer_lock = RwLock::new(new_peer);

		let insert_peer = Arc::new(peer_lock) ;
		{
			let mut peers = self.peers.write().unwrap() ;
			let insert_peer_clone = insert_peer.clone();
			peers.push(insert_peer_clone) ;
		}
	...
	pub fn close_peers(&self) {
		...
		for peer in peers.drain(..) {
			println!("[close_peers] peer.as_ptr(): {:p}", Arc::as_ptr(&peer));
			let peer = peer.read().unwrap() ;
...
fn main() {
	let p = Peers::new();
	let peers = Arc::new(p) ;
	println!("[main] peers.peers: {:p}", &(peers.peers));
	println!("[main] peers.peers.read(): {:p}", &(peers.peers.read().unwrap()));
